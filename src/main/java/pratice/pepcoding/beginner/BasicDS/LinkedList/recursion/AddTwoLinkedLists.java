...
// Source File Name:   AddTwoLinkedLists.java

package pratice.pepcoding.beginner.BasicDS.LinkedList.recursion;
...
public class AddTwoLinkedLists
{
    public static class LinkedList
    {
        ...
        private static int addListsHelper(Node one, int pv1, Node two, int pv2, LinkedList res)
        {
            ...
            if(pv1 > pv2)
            {
                int ocarry = addListsHelper(one.next, pv1 - 1, two, pv2, res);
                val = one.data + ocarry;
            } else
            if(pv1 < pv2)
            {
                int ocarry = addListsHelper(one, pv1, two.next, pv2 - 1, res);
                val = two.data + ocarry;
            } else
            {
                int ocarry = addListsHelper(one.next, pv1 - 1, two.next, pv2 - 1, res);
                val = one.data + two.data + ocarry;
            }
            res.addFirst(val % 10);
            return val / 10;
        }
        ...
        {
            LinkedList res = new LinkedList();
            int carry = addListsHelper(one.head, one.size, two.head, two.size, res);
            if(carry > 0)
                res.addFirst(carry);
            return res;
        }
