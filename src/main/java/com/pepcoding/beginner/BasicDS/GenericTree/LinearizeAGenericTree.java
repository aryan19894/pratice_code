...
// Source File Name:   LinearizeAGenericTree.java
...
public class LinearizeAGenericTree
{
    ...
    {
        Node child;
        for(Iterator iterator = node.children.iterator(); iterator.hasNext(); linearize(child))
            child = (Node)iterator.next();
    ...
    {
        for(; node.children.size() == 1; node = (Node)node.children.get(0));
        return node;
    }
    ...
    public static void linearize1(Node node)
    {
        ...
        for(int i = node.children.size() - 1; i > 0; i--)
        {
            Node shift = (Node)node.children.get(i);
            node.children.remove(i);
            Node addNode;
            for(addNode = (Node)node.children.get(i - 1); !addNode.children.isEmpty(); addNode = (Node)addNode.children.get(0));
            addNode.children.add(shift);
        }
