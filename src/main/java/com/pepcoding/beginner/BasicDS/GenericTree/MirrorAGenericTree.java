...
// Source File Name:   MirrorAGenericTree.java
...
public class MirrorAGenericTree
{
    ...
    public static void mirror1(Node node)
    {
        Stack st = new Stack();
        Node child;
        for(Iterator iterator = node.children.iterator(); iterator.hasNext(); st.add(child))
        {
            child = (Node)iterator.next();
            mirror(child);
        }
        ...
        node.children.clear();
        for(; !st.isEmpty(); node.children.add(st.pop()));
    }
